...
public class ExcelHelper
{
    ...
    public void DataTableToExcel(DataTable dt, int rows, int top, int left)
    {
        ...
        for (int i = 1; i <= sheetCount; i++)
        {
            ...
            string[,] ss = new string[row + 1, colCount + 1];
            //导入表头
            for (int c = 0; c < colCount; c++)
            {
                ss[0, c] = dt.Columns[c].ColumnName.ToString();
            }
            //数据导入
            for (int j = 0; j < row; j++)
            {
            ...
            range = range.get_Resize(row + 1, colCount + 1);
            range.Value = ss;
    ...
    /// <summary>
    /// 将DataTable数据写入Excel文件（自动分页）（数据是DataRow[]）
    /// </summary>
    /// <param name="dr">数据</param>
    /// <param name="rows">每个WorkSheet写入多少行数据</param>
    public void DataTableToExcel(DataRow[] dr, int rows, int top, int left)
    {
        int rowCount = dr.Length;		//DataTable行数
        int colCount = dr[0].Table.Columns.Count;	//DataTable列数
        sheetCount = this.GetSheetCount(rowCount, rows);	//WorkSheet个数
        //			StringBuilder sb;
        ...
        for (int i = 1; i <= sheetCount; i++)
        {
            ...
            for (int c = 0; c < colCount; c++)
            {
                ss[0, c] = dr[0].Table.Columns[c].ColumnName.ToString();
            }
            //数据导入
            for (int j = 0; j < row; j++)
            {
                for (int k = 0; k < colCount; k++)
                {
                    ss[j + 1, k] = dr[startRow + j][k].ToString();
                }
            }
    ...
    public void DataTableToExcel(IDataReader reader, int top, int left)
    {
